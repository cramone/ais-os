# Implementation Plan: ISign Feature + WeSign Multi-Document
...
**ISign** — sole signatory: the *current user* signs a document themselves, inline in an embedded iFrame.

---

## Part A: WeSign — Multi-Document Verification and Fixes
...
#### A1. Remove `debugger;` statement from `Send.cshtml`
...
#### A2. Verify `RequestUploader` JS component exists and handles multiple documents
...
#### A3. Verify `CreateEmbeddedWeSignViewModel` returns iFrame URL in JSON response
...
#### A4. Confirm InfoRouter document list supports multi-select → WeSign
...
## Part B: ISign — New Feature Implementation

### Overview

ISign is a sole-signatory embedded signing flow. The current user uploads a document to Secured Signing, then signs it themselves via an embedded iFrame. No external signers, no notify callback from SS server.
...
## Part C: Optional — ISign Audit Trail (DB Persistence)
...
## Part D: ISign Completion Callback (Post-Signing)
...
| Verify | WeSign multi-doc end-to-end flow (see Part A) |
