...
- **Process ticket persisted across recycles:** the process ticket is **stored in the dedicated app database** (associated with the `CleanupRun`). Because it is persisted rather than held only in memory, it **survives an IIS recycle / app restart** — on startup the app reloads the stored ticket and resumes the keep-alive. No re-authentication (and therefore no stored password) is needed, provided the app resumes within the ticket's 20-minute sliding window, which a normal recycle comfortably does.
