...
| Tag toolbar | Chips for every tag on the visible items, with counts, commonest first. Hover highlights the matching cards and dims the rest; click filters. Multiple tags **AND** together, so each one narrows — `plan` + `archive-cascade` is the plan for that workstream. Document-id tags are excluded: 34 of them in magiq-media alone, and the badge and search already cover ids |
