...
| HTML documents | Rendered in a sandboxed iframe, not inline. The design workstream's review and plan are complete HTML pages that define `--bg`, `--line`, `--ink` on `:root` — the same names Tower's theme uses — so injecting one inline repainted the whole app white |
| Tag toolbar | Chips for every tag on the visible items, with counts, **alphabetical** so a tag keeps its place as counts move. Each carries a colour derived from its own name — `review`/`plan`/`gate` pinned to their badge hues, the rest hashed (FNV-1a, hue from the low bits, one of three lightness steps from the high bits) so a tag looks the same every time and no two adjacent chips collide. Hover highlights the matching cards and dims the rest; click filters. Multiple tags **AND** together, so each one narrows — `plan` + `archive-cascade` is the plan for that workstream. Document-id tags are excluded: 34 of them in magiq-media alone, and the badge and search already cover ids |
| Dismissing | Clicking the selected card again toggles the panel shut; clicking away closes it. **Not** "away": inside the panel, another card (that switches selection), or a text field — clicking the search box means you intend to type, not to dismiss. A status tab does close it, since switching tab changes what you are looking at |
| Focus is shared | `body.focus-mode` is set by `_updateFocusMode()` from whether **anything** is lit — `.dep-target`, `.dep-source`, `.item-selected` or `.tag-match`. A pinned dependency and an open drilldown coexist; unpinning one does not undim the board while the other is still active |
