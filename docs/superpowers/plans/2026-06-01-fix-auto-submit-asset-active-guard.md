...
```bash
dotnet test "D:\source\github\sprbrk-standard\mgq-magiq-media\tests\modules\Catalog\Catalog.WriteModel.Tests" --filter "AssignAssetToRoleHandlerTests" -v 2>&1 | tail -15
```
...
```bash
dotnet test "D:\source\github\sprbrk-standard\mgq-magiq-media\tests\modules\Catalog\Catalog.WriteModel.Tests" --filter "AssignAssetToRoleHandlerTests" -v 2>&1 | tail -10
```
...
```bash
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" add src/modules/Catalog/Catalog.WriteModel/Commands/MediaItems/AssignAssetToRole/AssignAssetToRoleHandler.cs tests/modules/Catalog/Catalog.WriteModel.Tests/MediaItems/Commands/AssignAssetToRoleHandlerTests.cs
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" commit -m "fix(catalog): guard auto-submit on asset Active status — prevent premature publish when assets still processing"
```
