...
```bash
dotnet build "D:\source\github\sprbrk-standard\mgq-magiq-media\src\modules\ChangeRequests\ChangeRequests.Domain\ChangeRequests.Domain.csproj" 2>&1 | grep -E "error CS|Build succeeded|Build FAILED"
```
...
```bash
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" add src/modules/ChangeRequests/ChangeRequests.Domain/
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" commit -m "feat(change-requests): add ReviewSessionId + participant snapshot to ChangeRequest; guard AddComment to participants only"
```
...
```bash
dotnet build "D:\source\github\sprbrk-standard\mgq-magiq-media\src\modules\ChangeRequests\ChangeRequests.WriteModel\ChangeRequests.WriteModel.csproj" 2>&1 | grep -E "error CS|Build succeeded|Build FAILED"
```

- [ ] **Step 5: Commit**

```bash
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" add src/modules/ChangeRequests/ChangeRequests.WriteModel/Commands/CreateChangeRequest/
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" commit -m "feat(change-requests): add ReviewSessionId and ParticipantIds to CreateChangeRequestCommand"
```
...
```bash
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" add src/modules/ChangeRequests/ChangeRequests.WriteModel/IntegrationEvents/Consuming/Handlers/MediaItemPublicationRequestedEventHandler.cs
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" commit -m "feat(change-requests): pass ReviewSessionId and participant snapshot when creating comment thread CR"
```
...
```bash
dotnet test "D:\source\github\sprbrk-standard\mgq-magiq-media\tests\modules\Catalog\Catalog.WriteModel.Tests" --filter "PublishMediaItemHandlerTests" -v 2>&1 | tail -10
```
...
```bash
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" add src/modules/Catalog/Catalog.WriteModel/Commands/MediaItems/PublishMediaItem/PublishMediaItemHandler.cs tests/modules/Catalog/Catalog.WriteModel.Tests/MediaItems/Commands/PublishMediaItemHandlerTests.cs
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" commit -m "feat(catalog): pre-generate comment thread CR ID when publishing with reviewers"
```
...
```bash
dotnet build "D:\source\github\sprbrk-standard\mgq-magiq-media\src\modules\ChangeRequests\ChangeRequests.ReadModel\ChangeRequests.ReadModel.csproj" 2>&1 | grep -E "error CS|Build succeeded|Build FAILED"
dotnet build "D:\source\github\sprbrk-standard\mgq-magiq-media\src\modules\ChangeRequests\ChangeRequests.ReadModel.Infrastructure\ChangeRequests.ReadModel.Infrastructure.csproj" 2>&1 | grep -E "error CS|Build succeeded|Build FAILED"
```

- [ ] **Step 5: Commit**

```bash
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" add src/modules/ChangeRequests/ChangeRequests.ReadModel/
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" commit -m "feat(change-requests-read): project ReviewSessionId onto ChangeRequestDetailReadModel"
```
...
```bash
dotnet build "D:\source\github\sprbrk-standard\mgq-magiq-media\src\" 2>&1 | grep -E "error CS|Build succeeded|Build FAILED" | head -20
```
...
```bash
dotnet test "D:\source\github\sprbrk-standard\mgq-magiq-media\tests\integration\" --logger "console;verbosity=minimal" 2>&1 | tail -5
```
...
git -C "D:\source\github\sprbrk-standard\mgq-magiq-media" commit -m "fix: resolve any remaining build/test failures after CR+review integration"
```
