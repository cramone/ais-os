...
| `tower/deploy-cortex.sh` | One-command deploy: rebuild, restart, health-check, tag `deployed-tower`. Run on Cortex. **Revised 2026-07-05:** no longer does `git pull` first — see Data model above. **Corrected 2026-09-14:** the 07-05 revision was written up here but never applied to the script, which kept pulling for another two months. The drift surfaced when `pull --rebase` aborted a Tower deploy over unrelated uncommitted work under `projects/` — on a share several machines and agents write to, a dirty tree is the normal state, so the redundant pull was a standing outage risk. Now removed for real. The script also warns when `tower/` has uncommitted changes, since those get deployed but cannot be described by the `deployed-tower` tag. |
